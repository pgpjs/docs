...
In order to have a sidebar, you can create your own `_sidebar.md` (see [this documentation's sidebar](https://github.com/pgpjs/docs/blob/main/docs/_sidebar.md) for an example):
...
> [!IMPORTANT] PGPJS only looks for `_sidebar.md` in the current folder, and uses that, otherwise it falls back to the one configured using `window.$pgpjs.loadSidebar` config.
