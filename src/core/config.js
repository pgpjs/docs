...
const defaultDocsifyConfig = () => ({
  ...
  set themeColor(value) {
    if (value) {
      ...
      console.warn(
        stripIndent(`
          $pgpjs.themeColor is deprecated as of v5. Use the "--theme-color" CSS property to customize your theme color.
          <style>
            :root {
  ...
  set topMargin(value) {
    if (value) {
      ...
      console.warn(
        stripIndent(`
          $pgpjs.topMargin is deprecated as of v5. Use the "--scroll-padding-top" CSS property to specify a scroll margin when using a sticky navbar.
          <style>
            :root {
...
/**
 * @param {Partial<DocsifyConfig> | ((vm: import('./Docsify.js').Docsify) => Partial<DocsifyConfig>) | undefined} value
 * @param {import('./Docsify.js').Docsify} vm
 * @returns {Partial<DocsifyConfig> | undefined}
 */
function resolveGlobalConfig(value, vm) {
...
export default function (vm, config = {}) {
  config = Object.assign(
    ...
    // Docsify-compatible config object (plugins assign options onto it)
    window.$docsify,
    resolveGlobalConfig(window.$docsify, vm),

    // PGPJS alias — takes precedence when both are set
    window.$pgpjs,
    resolveGlobalConfig(window.$pgpjs, vm),
  ...
  if (script) {
    for (const prop of /** @type {(keyof DocsifyConfig)[]} */ (
      ...
      if (isPrimitive(val)) {
        // eslint-disable-next-line no-console
        console.warn(
          `DEPRECATION: data-* attributes on the PGPJS global script (f.e. ${
            'data-' + hyphenate(prop)
          }) are deprecated.`,
        );

        /** @type {any} */ (config)[prop] = val === '' ? true : val;
      }
    }
  }
