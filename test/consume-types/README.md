...
This is an example of a project that imports PGPJS via plain ESM in browser,
with type checking with TypeScript to prove that type definitions are working.

This ensures that downstream projects that consume PGPJS via ESM get
intellisense in their IDE (f.e. `Go To Definition` in VS Code will take the user
to PGPJS source code).
